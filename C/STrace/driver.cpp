...
NTSTATUS HandleDllLoad(PIRP Irp, PIO_STACK_LOCATION IrpStack) {
    ...
    if (!pluginData.isLoaded()) {
        ...
        if (pluginData.pInitialize) {
            ...
            PluginApis pluginApis(&MmGetSystemRoutineAddress, &LogPrint, &LogEtwEvent, &SetCallbackApi, &UnSetCallbackApi, &SetEtwCallback, &UnSetEtwCallback, &TraceAccessMemory, &SetTLSData, &GetTLSData);
