...
use winapi::shared::ntdef::{LIST_ENTRY, PKIRQL};
...
//extern "fastcall" {	fn IofCompleteRequest(Irp: PIRP, PriorityBoost:
// KPRIORITY_BOOST); }
extern "system" {
...
#[repr(u8)]
pub enum IRP_MJ {
    /// The operating system sends this request to open a handle to a file
    /// object or device object.
    CREATE,
    CREATE_NAMED_PIPE,
    /// Indicates that the last handle of the file object that is associated
    /// with the target device object has been closed and released. All
    /// outstanding I/O requests have been completed or canceled.
    /// See also `CLEANUP`.
    CLOSE,
    /// A user-mode application or Win32 component has requested a data transfer
    /// from the device. Or a higher-level driver has created and set up the
    /// read IRP.
    READ,
    /// A user-mode application or Win32 component has requested a data transfer
    /// to the device. Or a higher-level driver has created and set up the
    /// write IRP.
    WRITE,
    QUERY_INFORMATION,
    SET_INFORMATION,
    QUERY_EA,
    SET_EA,
    /// Indicates that the driver should flush the device's cache or its
    /// internal buffer, or, possibly, should discard the data in its
    /// internal buffer.
    FLUSH_BUFFERS,
    ...
    DIRECTORY_CONTROL,
    FILE_SYSTEM_CONTROL,
    /// An user-mode thread has called the Microsoft Win32 `DeviceIoControl`
    /// function, or a higher-level kernel-mode driver has set up the request.
    DEVICE_CONTROL,
    /// Some driver calls either `IoBuildDeviceIoControlRequest` or
    /// `IoAllocateIrp` to create a request.
    INTERNAL_DEVICE_CONTROL,
    /// Indicates that a file system driver is sending notice that the system is
    /// being shut down.
    SHUTDOWN,
    LOCK_CONTROL,
    /// Indicates that the last handle for a file object that is associated with
    /// the target device object has been closed (but, due to outstanding
    /// I/O requests, might not have been released). See also `CLOSE`.
    CLEANUP,
    CREATE_MAILSLOT,
    QUERY_SECURITY,
    SET_SECURITY,
...
/// The `IRP` structure is a partial opaque structure that represents an I/O
/// request packet.
#[repr(C)]
pub struct IRP {
    pub Type: u16,
    pub Size: u16,
    /// Pointer to an `MDL` describing a user buffer, if the driver is using
    /// direct I/O.
    ...
    // } Overlay
    /// Contains the entry point for a driver-supplied `Cancel` routine to be
    /// called if the IRP is canceled.
...
pub struct IO_STACK_LOCATION {
    /// The IRP major function code indicating the type of I/O operation to be
    /// performed.
    pub MajorFunction: u8,
    /// A subfunction code for `MajorFunction`.
    pub MinorFunction: u8,
    /// Request-type-specific values (see
    /// [DEVICE_FLAGS](../device_object/enum.DEVICE_FLAGS.html)).
    pub Flags: u8,
    ...
    /// representing the target physical, logical, or virtual device for which
    /// this driver is to handle the IRP.
    pub DeviceObject: PDEVICE_OBJECT,
    /// A pointer to a `FILE_OBJECT` structure that represents the file object,
    /// if any, that is associated with `DeviceObject` pointer.
    pub FileObject: PFILE_OBJECT,
    /// The following routine is invoked depending on the flags in the above
    /// `Flags` field.
    pub CompletionRoutine: PIO_COMPLETION_ROUTINE,
    /// The following is used to store the address of the context parameter that
    /// should be passed to the `CompletionRoutine`.
    pub Context: PVOID,
}
...
pub struct DeviceIoControl {
    pub OutputBufferLength: u64,
    pub InputBufferLength: u64,
    pub IoControlCode: u64,
    pub Type3InputBuffer: u64,
}

impl IRP {
    pub fn new(StackSize: i8) -> PIRP { unsafe { IoAllocateIrp(StackSize, false) } }

    pub fn with_quota(StackSize: i8) -> PIRP { unsafe { IoAllocateIrp(StackSize, true) } }

    pub fn free(&mut self) { unsafe { IoFreeIrp(self) }; }
    ...
    /// Indicates that the caller has completed all processing for a given I/O
    /// request and is returning the given IRP to the I/O manager.
    pub fn complete_request(&mut self, Status: Status) -> Status {
        self.IoStatus.Status = Status;
        unsafe { IoCompleteRequest(self, IO_PRIORITY::IO_NO_INCREMENT) };
        return Status;
    }

    /// Registers an `IoCompletion` routine, which will be called when the
    /// next-lower-level driver has completed the requested operation for
    /// the given IRP.
    pub fn set_completion(
        &mut self, CompletionRoutine: PIO_COMPLETION_ROUTINE, Context: PVOID,
    ...
    pub fn set_unconditional_completion(
        &mut self, CompletionRoutine: PIO_COMPLETION_ROUTINE, Context: PVOID,
    ) {
...
impl IO_STACK_LOCATION {
    ...
    pub fn ParametersDeviceIoControl(&mut self) -> &mut DeviceIoControl {
        unsafe { ::core::mem::transmute(&mut self.Parameters) }
    }
}

pub const fn CTL_CODE(device_type: u64, function: u64, method: u64, access: u64) -> u64 {
