...
extern "system" {
    pub fn IoCreateDevice(
        DriverObject: &mut DRIVER_OBJECT, DeviceExtensionSize: u32,
        DeviceName: *const UNICODE_STRING, DeviceType: u32, DeviceCharacteristics: u32,
        Exclusive: bool, DeviceObject: *mut *mut DEVICE_OBJECT,
    ) -> Status;

    pub fn IoDeleteDevice(DeviceObject: &mut DEVICE_OBJECT) -> Status;
    pub fn IoCreateSymbolicLink(
        SymbolicLinkName: &UNICODE_STRING, DeviceName: &UNICODE_STRING,
    ) -> Status;
    pub fn IoDeleteSymbolicLink(SymbolicLinkName: &UNICODE_STRING) -> Status;
}
...
/// The `DEVICE_OBJECT` structure is used by the operating system to represent a
/// device object.
#[repr(C)]
...
impl DEVICE_OBJECT {
    ...
    pub fn extension<T>(&mut self) -> &mut T { unsafe { &mut *(self.DeviceExtension as *mut T) } }
}
...
pub type PDRIVER_DISPATCH =
    Option<extern "system" fn(DeviceObject: &mut DEVICE_OBJECT, Irp: &mut IRP) -> Status>;
